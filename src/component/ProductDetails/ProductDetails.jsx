  ...
  <main className="my-32">
    <div className="container mx-auto px-6">
      <div className="md:flex md:items-center">
        <div className="w-full mx-auto md:w-3/12 lg:h-96">
        <ProductImagesSlider images={productDetails?.images} />
        </div>
        <div className="w-full max-w-lg mx-auto mt-5 md:ml-8 md:mt-0 md:w-9/12">
          <h3 className="text-green-700  text-2xl py-3 font-sans">{productDetails?.title}</h3>
          <span className="text-gray-950 mt-3 text-xl">{productDetails?.price}<span className='text-green-700'> EGp</span></span>
          <hr className="my-3" />
          
          <div className="mt-3">
            <label className="text-green-700 text-lg font-sans" htmlFor="count">Category:</label>
            <h3 className='text-gray-950 font-sans'>{productDetails?.category.name}</h3>
          </div>
          <div className="mt-3">
            <label className="text-green-700 text-lg font-sans" htmlFor="count">SubCategory:</label>
            <h3 className='text-gray-950 font-sans'>{productDetails?.subcategory[0].name}</h3>
          </div>
          <div className="mt-3">
            <label className="text-green-700 text-lg font-sans" htmlFor="count">Description:</label>
            <h3 className='text-gray-950 font-sans'>{productDetails?.description}</h3>
          </div>
          <div className="mt-3">
            <label className="text-green-700 text-lg font-sans" htmlFor="count">Brands:</label>
            <h3 className='text-gray-950 font-sans'>{productDetails?.brand.name}</h3>
          </div>
          <div className="mt-3">
            <RatingStars rating={productDetails?.ratingsAverage ?? 0} />
          </div>
          <div className="flex items-center mt-6">
            <button className="px-8 py-2 bg-green-700 text-white text-sm font-medium rounded hover:bg-green-600 focus:outline-none focus:bg-indigo-500">Order Now</button>
            <button onClick={()=>{addProductToCart(productDetails._id);}} className="mx-2 text-green-600 border rounded-md p-2 hover:bg-green-200 focus:outline-none">
